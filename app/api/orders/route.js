...
export async function GET(req) {
  ...
  try {
    const { rows } = await query(
      `select o.*, coalesce(json_agg(json_build_object('name', oi.name, 'qty', oi.qty, 'price', oi.price)) filter (where oi.id is not null), '[]') as items
       from orders o
...
export async function POST(req) {
  ...
  try {
    ...
    const { rows } = await client.query(
      `insert into orders (code, customer, total, payment_method)
       values ($1,$2,$3,$4) returning id, code, created_at`,
      [code, customer, total, paymentMethod],
    )
    const order = rows[0]
    ...
    return NextResponse.json(
      { id: order.code, total: Number(total), paymentMethod, customer, items },
      { status: 201 },
    )
  } catch (error) {
