...
export async function registerProductView(productId, visitorId) {
  const { rowCount } = await query(
    `insert into product_view_events (product_id, visitor_id)
     select $1, $2
     where not exists (
       select 1 from product_view_events
       where product_id = $1 and visitor_id = $2 and created_at >= date_trunc('day', now())
     )`,
    [productId, visitorId],
  )
  if (rowCount) await incrementProductView(productId)
}
...
export async function getOrderByCode(code) {
  const { rows } = await query(
    `select o.code, o.customer, o.total, o.payment_method, o.status, o.created_at,
            to_char(o.estimated_date, 'YYYY-MM-DD') as estimated_date,
