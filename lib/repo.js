...
export async function createTestimonial(data) {
  const { rows } = await query(
    `insert into testimonials (name, role, text, rating, active, sort_order)
     values ($1,$2,$3,$4,$5,$6) returning *`,
    [data.name, data.role || null, data.text, Number(data.rating) || 5, data.active !== false, Number(data.sortOrder) || 0],
  )
...
export async function updateTestimonial(id, data) {
  const { rows } = await query(
    `update testimonials set name=$1, role=$2, text=$3, rating=$4, active=$5, sort_order=$6
     where id=$7 returning *`,
    [data.name, data.role || null, data.text, Number(data.rating) || 5, data.active !== false, Number(data.sortOrder) || 0, id],
  )
  return mapTestimonial(rows[0])
}
