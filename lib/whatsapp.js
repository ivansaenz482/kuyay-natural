...
const paymentLabel = (id) => PAYMENT_LABELS[id] || id

// Numero unico de la tienda (se puede cambiar desde el panel admin)
export const DEFAULT_NUMBERS = [
  process.env.NEXT_PUBLIC_WHATSAPP_PRIMARY || '593967598834',
].filter(Boolean)

export function formatPhone(value) {
  const d = String(value || '').replace(/\D/g, '')
  if (d.length === 12 && d.startsWith('593')) {
    return `+593 ${d.slice(3, 5)} ${d.slice(5, 8)} ${d.slice(8, 12)}`
  }
  if (d.length === 10) {
    return `+593 ${d.slice(0, 2)} ${d.slice(2, 5)} ${d.slice(5, 10)}`
  }
  return d ? `+${d}` : ''
}
...
export function bankAccountsMessage(accounts = [DEFAULT_BANK]) {
  const blocks = (accounts.length ? accounts : [DEFAULT_BANK]).map((a) =>
    [
      `• ${a.banco} — ${a.tipo}`,
      `  Cuenta: ${a.numero}`,
      `  Titular: ${a.titular}`,
      `  RUC/CI: ${a.identificacion}`,
    ].join('\n'),
  )
  return ['Kuyay Natural 🌿', 'Datos para pago por transferencia:', '', ...blocks].join('\n')
}
